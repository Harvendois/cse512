...
class OrdinalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        alpha = 0.3     # distance penalty weight
...
def train_model(train_loader, test_loader, num_classes, num_epochs=20, lr=4e-5):
    ...
    # Load best saved model for fine-tuning
    model.load_state_dict(torch.load("./best_models/model61_acc60.pth"))
    print("🔁 Loaded best model for fine-tuning.")


    criterion = OrdinalLoss(num_classes=num_classes, class_weights=True, smoothing=0.2)
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=2e-6)
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), "best_model_ordinal.pth")
