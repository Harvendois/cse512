...
class FloatLabelWrapper(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.tensor(label / 10.0, dtype=torch.float)  # Normalize from [0,10] → [0,1]
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.images[idx], torch.tensor(self.labels[idx] / 10.0, dtype=torch.float)  # Normalize
...
def train_model(train_loader, test_loader, num_epochs=30, lr=1e-3):
    ...
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=1e-6)
    ...
    all_preds = np.array(all_preds) * 100
...
def plot_predictions(y_true, y_pred):
    ...
    # plot the regression line with data above
