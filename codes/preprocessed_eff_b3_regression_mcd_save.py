...
def train_model(train_loader, test_loader, num_epochs=20, lr=1e-3):
    ...
    for epoch in range(num_epochs):
        ...
        if total_loss < best_loss:
            ...
            torch.save(model.state_dict(), "best_model_mc_dropout.pth")
...
def evaluate_with_uncertainty(model, test_loader, num_samples=20):
    model.load_state_dict(torch.load("best_model_mc_dropout.pth"))
...
def show_regression_results(y_true, y_pred, y_std, class_names):
    ...
    plt.title("MC Dropout Regression Confusion Matrix")
    plt.tight_layout()
    plt.show()
    ...
    plt.title("MC Dropout Predictions ± Uncertainty")
    plt.tight_layout()
    plt.show()
